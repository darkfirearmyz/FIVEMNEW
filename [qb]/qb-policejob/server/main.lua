...
RegisterNetEvent('qb-policejob:server:trash', function()
    ...
    if not Player then return end
    if Player.PlayerData.job.type ~= 'leo' then return end
    exports['lj-inventory']:OpenInventory(src, 'policetrash', {
        maxweight = 4000000,
        slots = 300,
    })
end)
...
RegisterNetEvent('qb-policejob:server:evidence', function(currentEvidence)
    ...
    if not Player then return end
    if Player.PlayerData.job.type ~= 'leo' then return end
    exports['lj-inventory']:OpenInventory(src, currentEvidence, {
        maxweight = 4000000,
        slots = 500,
    })
end)
...
QBCore.Functions.CreateUseableItem('moneybag', function(source, item)
    ...
    if not exports['lj-inventory']:RemoveItem(src, 'moneybag', 1, item.slot, 'qb-policejob:moneybag') then return end
