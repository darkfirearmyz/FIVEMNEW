...
RegisterNetEvent('qb-mechanicjob:server:removeItem', function(part, amount)
    ...
    if not exports['lj-inventory']:RemoveItem(src, part, amount, false, 'qb-mechanicjob:server:removeItem') then DropPlayer(src, 'qb-mechanicjob:server:removeItem') end
    TriggerClientEvent('lj-inventory:client:ItemBox', src, QBCore.Shared.Items[part], 'remove')
end)
