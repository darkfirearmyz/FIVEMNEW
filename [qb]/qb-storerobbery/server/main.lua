...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        ...
        exports['lj-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            exports['lj-inventory']:AddItem(src, 'stickynote', 1, false, info, 'qb-storerobbery:server:takeMoney')
            TriggerClientEvent('lj-inventory:client:ItemBox', src, QBCore.Shared.Items['stickynote'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    exports['lj-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:SafeReward')
    TriggerClientEvent('lj-inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add')
    local luck = math.random(1, 100)
    local odd = math.random(1, 100)
    if luck <= 10 then
        exports['lj-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'qb-storerobbery:server:SafeReward')
        TriggerClientEvent('lj-inventory:client:ItemBox', src, QBCore.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['lj-inventory']:AddItem(src, 'goldbar', 1, false, false, 'qb-storerobbery:server:SafeReward')
            TriggerClientEvent('lj-inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:removeAdvancedLockpick', function()
    ...
    if not Player then return end
    exports['lj-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'qb-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('qb-storerobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['lj-inventory']:RemoveItem(source, 'lockpick', 1, false, 'qb-storerobbery:server:removeLockpick')
end)
